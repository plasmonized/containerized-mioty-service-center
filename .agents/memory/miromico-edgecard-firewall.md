...
**How to apply:** When a mioty base station behind this gateway won't connect, either map the service center's external Docker port to 16017 (blocked on this NAS: Synology DSM nginx occupies 16017) or persist an ACCEPT rule on the gateway. Chosen fix: appended `iptables -I FORWARD 1 -s 172.30.1.2 -p tcp --dport 16020 -j ACCEPT` to `/usr/local/bin/mioty-bs-only.sh` (runs on every boot via mioty-bs-only.service); verified reboot-safe. May be wiped by a gateway firmware update — re-add if BS stops connecting after one. Check FORWARD counters early: it's the fastest way to see if the card is trying. tcpdump is not installed on the gateway; use iptables packet counters or /dev/tcp tests instead. The card's HTTP API is at http://172.30.1.2/cgi-bin/{config,status} (gateway shell only).
