...
class TLSServer:
    ...
    async def send_attach_request(self, writer: asyncio.streams.StreamWriter, sensor: dict[str, Any]) -> None:
        ...
        try:
                ...
                if reg_info.get("status") == "registered":
                    ...
                    if bs_eui in existing_bases:
                        validation_warnings.append(
                            f"Sensor {sensor['eui']} already registered to base station {bs_eui}"
                        )
                        logger.warning("   ⚠️  Re-registering sensor to same base station")
                    else:
                        validation_warnings.append(
                            f"Sensor {sensor['eui']} already registered to {len(existing_bases)} other base station(s): {existing_bases}"
                        )
                        logger.warning("   ⚠️  Adding registration to additional base station")
            ...
            if not validation_errors:
                ...
                logger.info("   Tracking request for correlation with response")
                logger.info("   Awaiting response from base station...")
                logger.info("   =====================================")
                ...
                logger.error("   ❌ Attach request NOT sent due to validation failures")
                logger.error("   =====================================")
        ...
        for i, sensor in enumerate(self.sensor_config, 1):
            try:
                if writer not in self.connected_base_stations:
                    logger.warning(
                        f"   ⚠️ Writer no longer in connected base stations - aborting batch attach for {bs_eui}"
                    )
                    break
    ...
    async def send_detach_request(self, writer: asyncio.streams.StreamWriter, sensor_eui: str) -> bool:
        ...
        try:
            ...
            logger.info("   📤 Transmitting detach request...")
            logger.info(f"     Message size: {len(full_message)} bytes")
            ...
            logger.error("❌ CRITICAL ERROR during detach request")
    ...
    async def detach_all_sensors(self) -> int:
        ...
        logger.info("🔌 DETACHING ALL SENSORS from all base stations")
        ...
        logger.info("✅ BULK DETACH completed")
    ...
    def clear_all_sensors(self) -> None:
        ...
        logger.info("✅ ALL SENSORS CLEARED")
    ...
    def detach_all_sensors_sync(self) -> int:
        ...
        try:
            logger.info("🔌 SYNC DETACHING ALL SENSORS from all base stations")
    ...
    def attach_all_sensors_sync(self) -> int:
        ...
        try:
            logger.info("🔗 SYNC ATTACHING ALL SENSORS to all base stations")
                    ...
                    for sensor in self.sensor_config:
                        for writer in list(self.connected_base_stations.keys()):
                            try:
                                ...
                                total_attachments += 1
                                logger.info(
                                    f"   ✅ Attach request sent for {sensor['eui']} to {self.connected_base_stations[writer]}"
                                )
                                # Small delay between requests
                                await asyncio.sleep(0.1)
                            except Exception as e:
                                logger.error(
                                    f"   ❌ Failed to send attach for {sensor['eui']} to {self.connected_base_stations.get(writer, 'unknown')}: {e}"
                                )
                ...
                if self.connected_base_stations:
                    ...
                    for writer in list(self.connected_base_stations.keys()):
                        try:
                            ...
                            logger.warning(
                                f"   ⚠️ Failed to send status to {bs_eui} ({fail_count}/{self.BS_STATUS_FAILURE_THRESHOLD}): {e}"
                            )
                    ...
                    logger.info("📊 STATUS REQUEST CYCLE COMPLETED")
                else:
                    logger.debug("📊 No base stations connected - skipping status requests")
            ...
            while True:
                ...
                for message in decode_messages(data):
                    ...
                    if msg_type == "con":
                        ...
                        logger.info(
                            f"   Uptime: {uptime_hours:02d}:{uptime_minutes:02d}:{uptime_secs:02d} ({uptime_seconds}s)"
                        )
                        ...
                        logger.info("📤 MQTT PUBLICATION - BASE STATION STATUS")
                        ...
                        if pending_request:
                            ...
                            logger.info(
                                f"     Network Key: {sensor_config['nwKey'][:8]}...{sensor_config['nwKey'][-8:]}"
                            )
                            ...
                            logger.info(
                                f"   All base stations for sensor: {self.registered_sensors[eui_key]['base_stations']}"
                            )
                            logger.info(
                                f"   Total registered sensors: {len([k for k in self.registered_sensors.keys() if not k.endswith('_failure')])}"
                            )
                            ...
                            logger.warning(
                                f"   Available pending requests: {list(self.pending_attach_requests.keys())}"
                            )
                            logger.warning("   This could indicate:")
                            logger.warning("     - Response arrived after timeout")
                            logger.warning("     - Duplicate response")
                            logger.warning("     - Base station sent unsolicited response")
                            ...
                            if recent_requests:
                                ...
                                logger.warning("   🔄 FALLBACK: Using most recent pending request")
                        ...
                        if is_duplicate:
                            ...
                            if snr > existing_message["snr"]:
                                ...
                                logger.debug(
                                    f"   🔽 DEDUPLICATION: Filtered duplicate message for {eui} with lower SNR ({snr:.2f} dB <= {existing_message['snr']:.2f} dB)"
                                )
                    ...
                    # Variable MAC (VM) Sub-Channel Message Handlers
                        ...
                        if meter_info:
                            ...
                            logger.info(
                                f"   OMS Meter: {meter_info['manufacturer_code']} Serial {meter_info['serial']} ({meter_info['device_type_name']})"
                            )
                            import time
    ...
    async def process_deduplication_buffer(self) -> None:
        ...
        while True:
            ...
            for key, value in list(
                self.deduplication_buffer.items()
            ...
            for message_key, message_data in messages_to_publish:
                ...
                logger.info("📤 PUBLISHING DEDUPLICATED MESSAGE")
                logger.info("   =====================================")
            ...
            oldest_allowed_time = current_time - (
                self.deduplication_delay * 2
    ...
    async def heartbeat_monitor(self) -> None:
        ...
        logger.info(
            f"💓 HEARTBEAT MONITOR STARTED (check every {self.HEARTBEAT_CHECK_INTERVAL}s, offline after {self.HEARTBEAT_OFFLINE_MULTIPLIER}x avg interval)"
        )
    ...
    async def auto_detach_monitor(self) -> None:
        ...
        logger.info("🕐 AUTO-DETACH MONITOR STARTED")
        logger.info(
        ...
        try:
            while True:
                ...
                if sensors_to_detach:
                    logger.info("🕐 AUTO-DETACH MONITOR CYCLE COMPLETE")
                    logger.info(f"   Sensors auto-detached: {len(sensors_to_detach)}")
                elif len(self.registered_sensors) > 0:
                    logger.debug(
                        f"🕐 AUTO-DETACH MONITOR: All {len(self.registered_sensors)} sensors within activity thresholds"
                    )

        except asyncio.CancelledError:
            logger.info("🕐 AUTO-DETACH MONITOR CANCELLED")
            raise
        except Exception as e:
    ...
    async def send_inactivity_warning(
        self, eui_key: str, inactive_time: float, warning_timeout: float, detach_timeout: float
    ) -> None:
        ...
        logger.warning("⚠️  SENSOR INACTIVITY WARNING")
    ...
    async def auto_detach_inactive_sensor(self, eui_key: str, inactive_time: float) -> None:
        ...
        if success:
            ...
            if self.mqtt_out_queue:
                ...
                await self.mqtt_out_queue.put(
                    {"topic": f"ep/{eui.upper()}/status", "payload": json.dumps(detach_payload)}
                )
            ...
            if self.mqtt_out_queue:
                ...
                await self.mqtt_out_queue.put(
                    {"topic": f"ep/{eui.upper()}/error", "payload": json.dumps(failure_payload)}
                )
    ...
    async def periodic_vm_status_query(self) -> None:
        ...
        if self.connected_base_stations:
            ...
            if self.vm_capable_base_stations:
                logger.info(
                    f"✅ VM DISCOVERY COMPLETE - Found {len(self.vm_capable_base_stations)} VM-capable base stations:"
                )
        ...
        while True:
            try:
                ...
                if self.vm_periodic_status_enabled and self.vm_capable_base_stations:
                    logger.info(
                        f"📊 PERIODIC VM STATUS QUERY - {len(self.vm_capable_base_stations)} VM-capable base stations"
                    )
    ...
    def reload_sensor_config(self) -> None:
        ...
        try:
            with open(self.sensor_config_file) as f:
    ...
    def get_oms_stats(self) -> dict[str, Any]:
        ...
        return {"total_meters": total_meters, "total_messages": total_messages}
        ...
        for sensor in self.sensor_config:
            if sensor["eui"].upper() == eui_upper:
                ...
                try:
                    with open(self.sensor_config_file, "w") as f:
                        json.dump(self.sensor_config, f, indent=4)
                    logger.debug("✅ Preferred downlink path saved successfully")
                except Exception as e:
    ...
    async def process_sensor_config_message(self, message: dict) -> None:
        ...
        logger.info("🔧 PROCESSING SENSOR CONFIGURATION MESSAGE")
        logger.info(f"   EUI: {message.get('eui', 'unknown')}")
