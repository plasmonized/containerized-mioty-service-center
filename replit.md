...
- **v1.664 - Per-BS Operation ID Fix**: Fixed critical bug causing non-VM base stations (Weptech AVA, Miromico Edge, RAK WisGate) to continuously disconnect with "invalid op id" errors. Root cause: global opID counter shared across all BS connections caused non-sequential IDs per connection. Fix: each BS connection now gets its own sequential opID counter starting from 1, properly initialized on connect and cleaned up on disconnect. Also fixed inconsistent opID direction (attach decremented, status incremented) - all now increment consistently.
