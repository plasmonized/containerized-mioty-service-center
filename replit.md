...
- **v1.698 - Server-Zertifikat mit Subject Alternative Names (SAN-Fix für BS-Verbindung)**: Basisstationen mit moderner TLS-Implementierung (ifm und andere) ignorieren das CN-Feld und prüfen ausschließlich SANs zur Hostname-Verifikation. Das bisher generierte Server-Zertifikat hatte keine SANs → Handshake-Abbruch bei allen BS. Neu: `generate_certificates()` ermittelt automatisch alle relevanten IPs (Container-IP, Outbound-IP, Docker-Gateway via `/proc/net/route`) und trägt sie als `IP:x.x.x.x`-SANs ein. Zusätzlich neue `.env`-Variable `SC_HOST_IP` für die explizite Angabe der Host-IP (empfohlen bei Docker). Zertifikatslaufzeit 1 Jahr → 10 Jahre (wie CA). SANs werden im Erfolgs-Response angezeigt. **Aktion erforderlich:** Einmal neue Zertifikate generieren (Web-UI → Zertifikate → Neu generieren), neue `ca_cert.pem` auf BS laden, Container neu starten.

- **v1.696 - TLS-Fix für Python 3.12+ / OpenSSL 3.x (CA keyUsage)**: Neuere Docker-Images mit Python 3.12/OpenSSL 3.x verweigern TLS-Handshakes, wenn das CA-Zertifikat keine `keyUsage: keyCertSign`-Extension hat. Fix: `VERIFY_X509_STRICT`-Flag wird in beiden TLS-Kontexten (BSSCI + SCACI) deaktiviert — alte CA-Zertifikate funktionieren weiter, alle anderen Prüfungen (Chain, Ablauf, CN) bleiben aktiv. Neue CA-Zertifikate (Web-UI → Zertifikate → Neu generieren) enthalten jetzt korrekt `basicConstraints=CA:true`, `keyUsage=keyCertSign,cRLSign` und laufen 10 Jahre. 161 Tests grün.

- **v1.695 - SCACI-Toggle überlebt Neustart trotz veralteter gemounteter Config**: Neues Modul `config_compat.py` mit `get_config()`: Fehlt ein Attribut in einer (im Docker gemounteten, veralteten) `bssci_config.py`, wird der Wert direkt aus der Umgebung/`.env` gelesen statt stumpf auf den Default zurückzufallen. Betroffen waren `SCACI_ENABLED`, `SCACI_PORT`, `SCACI_HOST`, SCACI-Zertifikatspfade und `MQTT_ENABLED` — der SCACI-Haken auf der Config-Seite blieb nach Neustart deaktiviert, weil die alte gemountete Config die neue Option gar nicht kannte. Warnung im Log weist jetzt auf die veraltete Config-Datei hin. 155 Tests grün.

- **v1.694 - Robustere Startphase & CI grün (Ruff + Mypy)**: (1) Direkte Zugriffe auf `DEDUPLICATION_DELAY` und `SCACI_PORT` beim Start verwenden jetzt Fallback-Defaults (`getattr`) — eine im Docker gemountete, veraltete `bssci_config.py` kann den Service-Start nicht mehr verhindern. (2) GitHub-CI-Fehler behoben: TLSServer akzeptiert `Optional`-Queues (MQTT-deaktiviert-Modus) mit None-Guards in `process_mqtt_messages`/`process_mqtt_command`; MQTTClient wird nur noch mit echten Queues konstruiert; RUF012 in SCACI-Tests (Class-Level-Dicts → `__init__`); ruff format auf 4 Dateien angewendet. Mypy und Ruff laufen fehlerfrei, 155 Tests grün.

- **v1.693 - AC-Zertifikate löschbar**: Auf der App-Centers-Seite gibt es in der Zertifikatstabelle jetzt neben dem ZIP-Download einen Mülleimer-Button zum Löschen eines AC-Zertifikats (mit Bestätigungsdialog). Neuer API-Endpunkt `DELETE /api/ac-certificates/<name>` entfernt das komplette Verzeichnis `certs/ac_{name}` (Zertifikat, Key, ZIP). Zugriff nur mit `can_manage_certificates`-Berechtigung; Namens-Sanitisierung verhindert Path-Traversal.
